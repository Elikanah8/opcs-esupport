...
import { LayoutDashboard, ListTodo, CheckCircle, Settings, LogOut, User, Mail, Building } from "lucide-react";
        ...
        <nav style={{ flex: 1, padding: 16, display: "flex", flexDirection: "column", gap: 4 }}>
          {navItems.map((item, i) => (
            <button
              ...
              style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderRadius: 12, border: "none", cursor: "pointer", backgroundColor: item.href === "/intern/settings" ? "rgba(255,255,255,0.15)" : "transparent", color: item.href === "/intern/settings" ? "white" : "rgba(255,255,255,0.55)", fontSize: 14, fontWeight: 600, textAlign: "left", width: "100%" }}
            >
              <item.icon size={18} />
              {item.label}
            </button>
          ...
          <div style={{ maxWidth: 600 }}>
            <div style={{ backgroundColor: "white", borderRadius: 16, padding: 32, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", marginBottom: 24 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 32 }}>
                ...
                <div>
                  <p style={{ fontSize: 18, fontWeight: 700, color: "#1E293B" }}>{user?.name || "Intern"}</p>
                  <p style={{ fontSize: 13, color: "#94A3B8" }}>ICT Intern / Attachee</p>
                </div>
              </div>
